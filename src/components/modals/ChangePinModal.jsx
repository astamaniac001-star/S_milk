...
export function ChangePinModal({ onClose, showToast }) {
    ...
    const [busy, handleSubmit] = useBusy(async () => {
        if (newPin.length !== 6) return showToast("New PIN must be exactly 6 digits", "error");
        if (newPin !== confirmPin) return showToast("New PINs do not match", "error");
        if (currentPin === newPin) return showToast("New PIN must be different from current PIN", "error");

        try {
            await callApi("rotatePIN", { currentPin, newPin });
            showToast("PIN updated successfully! Use it next time you log in.", "success");
            onClose();
        } catch (e) {
    ...
    const handleChange = (setter) => (e) => setter(e.target.value.replace(/\D/g, "").slice(0, 6));
        ...
        <Modal title="Change Operator PIN" onClose={onClose}>
            <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                ...
                <div style={{ display: "flex", gap: 8, marginTop: 8 }}>
                    <Btn onClick={handleSubmit} disabled={busy || currentPin.length < 6 || newPin.length < 6 || confirmPin.length < 6}>
